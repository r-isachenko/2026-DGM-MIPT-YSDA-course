...
\begin{frame}{Recap of Previous Lecture}
	...
	\[
		\bmu_t(\bx_0, \bx_1) = t \bx_1 + (1 - t) \bx_0 \quad
	...
	\begin{block}{Uniform vs. Absorbing Transition Matrix}
        \begin{table}[h!]
            ...
            \begin{tabular}{lcc}
                ...
                \midrule
                $\bQ_t$
                    & $(1-\beta_t)\bI + \beta_t \bU$ 
                    & $(1-\beta_t)\bI + \beta_t \be_m\bone^\top$ \\[4pt]
                $\bQ_{1:t}$
                    & $\bar\alpha_t \bI + (1-\bar\alpha_t)\bU$
                    & $\bar\alpha_t \bI + (1-\bar\alpha_t)\be_m\bone^\top$ \\[4pt]
                $\bQ_{1:\infty}$
                    & $\bU$
                    & $\Cat(\be_m)$ \\[4pt]
                Interpretation
...
\begin{frame}{Recap of Previous Lecture}
	...
	\end{block}
	\begin{itemize}
		\item The reverse process $\pt(\bx_{t-1} | \bx_t) = \Cat\bigl( \bpi_{\btheta}(\bx_t, t) \bigr)$ is a learned categorical distribution.
		\item Minimizing $\cL_t$ w.r.t.~$\btheta$ is equivalent to minimizing the cross-entropy
		\[
			\bbE_{q(\bx_t | \bx_0)}
...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{itemize}
		...
		\item Each factor conditions on the entire noisy sequence ${\color{violet}\bX_t}$ --- this is exactly the \textbf{masked language modeling} pattern.
	\end{itemize}
...
\begin{frame}{Absorbing Diffusion}
	...
	\begin{block}{Conditioned reverse distribution}
		...
		\[
			...
			\begin{cases}
				[\bx_{t-1} = \bx_t], 
				& \text{if } \bx_t \neq \be_m, \\
				\rho_t[\bx_{t-1} = \bx_0] + (1-\rho_t)[\bx_{t-1} = \be_m], 
				& \text{if } \bx_t = \be_m,
			\end{cases}
		\]
		where
	...
	\eqpause
	\begin{itemize}
		\item If $\bx_t \neq \be_m$, then the token must be unchanged:
		$\bx_{t-1} = \bx_t$.
		\item Observing an unmasked token at time $t$ fixes the entire history:
		$\bx_{t-1} = \bx_t = \cdots = \bx_0$.
		\eqpause
		\item If $\bx_t = \be_m$, the previous token may be either clean or masked.
...
\begin{frame}{Absorbing Diffusion}
	...
	\vspace{-0.3cm}
    \eqpause
    \begin{block}{Objective: sequence-level $\cL_t$}
        \vspace{-0.3cm}
        \[
            \cL_t
            =
            \bbE_{q(\bX_t| \bX_0)}
            \sum_{i=1}^m
            \rho_t [\bx_t^i=\be_m] \left[-\log \pt(\bx_0^i | \bX_t)\right]
			+ \text{const}
        \]
    \end{block}
\end{frame}
...
\begin{frame}{Algorithm: Discrete-Time Absorbing Diffusion}
	...
	\begin{block}{Training}
		\begin{enumerate}
			...
			\item Sample the corrupted sequence by independent masking:
			\vspace{-0.3cm}
			\[
				\bx_t^i =
				\begin{cases}
...
\begin{frame}{Algorithm: Discrete-Time Absorbing Diffusion}
	...
	\begin{block}{Sampling}
		\begin{enumerate}
			\item Initialize $\bX_T \leftarrow \be_m \bone^\top$ (fully masked).
			\item Iterate for $t = T, T-1, \ldots, 1$:
			\begin{enumerate}
				\item Predict $\pt(\bx_0^i | \bX_t)$ for all positions.
				\item For each masked position ($\bx_t^i = \be_m$):
				\vspace{-0.3cm}
				\[
					\bx_{t-1}^i =
					\begin{cases}
...
\begin{frame}{Discrete ELBO Revisited}
    ...
    \[
        ...
        \bbE_{q(\bX_t | \bX_0)}
        \sum_{i=1}^m
        {\color{violet}\rho_t}[\bx_t^i=\be_m]
	...
	\begin{block}{Training}
		\begin{enumerate}
			...
			\[
				...
				\begin{cases}
					\be_m, & \text{with prob. }{\color{violet}\lambda},\\
			...
			\[
				\cL
				=
				{\color{violet}w(\lambda)} \sum_{i=1}^m [\bx_{\color{violet}\lambda}^i=\be_m]\,
...
\begin{frame}{Algorithm: Masked Diffusion Language Model (MDLM)}
	...
	\begin{block}{Sampling}
		\begin{enumerate}
			\item Initialize $\bX_{{\color{violet}\lambda_1}} \leftarrow \be_m \bone^\top$ (fully masked).
			\item Iterate for $\ell = 1, \ldots, L-1$ over a decreasing schedule ${\color{violet}1 = \lambda_1 > \lambda_2 > \cdots > \lambda_L = 0}$:
			\begin{enumerate}
				\item Predict $\pt(\bx_0^i | \bX_{{\color{violet}\lambda_\ell}})$ for all positions.
				\item For each masked position ($\bx_{{\color{violet}\lambda_\ell}}^i = \be_m$):
				\vspace{-0.3cm}
				\[
					...
					\begin{cases}
						...
						\be_m, & \text{with prob. } {\color{violet}\tfrac{\lambda_{\ell+1}}{\lambda_\ell}}.
					\end{cases}
				\]
				\vspace{-0.5cm}
...
\begin{frame}{Discrete Diffusion (L12--L13)}
	\begin{block}{Forward discrete process (L12)}
		...
		\begin{itemize}
			...
			\item \textbf{Absorbing}: $\bQ_t = (1-\beta_t)\bI + \beta_t\,\be_m \bone^{\top}$ (mask token)
		\end{itemize}
	\end{block}
