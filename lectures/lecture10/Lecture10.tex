...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{block}{Continuous-Time Dynamics (Neural ODE)}
		...
		\begin{align*}
			...
			\bpsi_{t_1}(\bx_0) &= \int^{t_1}_{t_0} \bv_{\btheta}(\bx(t), t) d t  + \bx_0 \approx {\color{teal}\ODESolve_v(\bx_0, \btheta, t_0, t_1)}.
		\end{align*}
		\vspace{-0.5cm}
	\end{block}
	\begin{itemize}
...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{block}{Theorem (Picard)}
		If $\bv$ is continuously differentiable in $\bx$ (with bounded derivative) and continuous in $t$, then the ODE has a \textbf{unique} solution.
	\end{block}
...
\begin{frame}{Recap of Previous Lecture}
    \myfootnotewithlink{https://www.stats.ox.ac.uk/~teh/research/compstats/WelTeh2011a.pdf}{Welling M. Bayesian Learning via Stochastic Gradient Langevin Dynamics, 2011}
 	...
 	\begin{block}{Langevin SDE (Special Case)}
 		\vspace{-0.3cm}
 		\[
 			d \bx = {\color{violet}\frac{1}{2} \frac{\partial}{\partial \bx} \log \pt(\bx)} d t + {\color{teal} 1 } \cdot d \bw
 		\]
 		\vspace{-0.3cm}
 	\end{block}
...
\begin{frame}{Summary}
	\begin{itemize}
		\item Every SDE admits a corresponding probability flow ODE that follows the same probability path $p_t(\bx)$.
		\vfill
		\item Converting SDEs to PF-ODEs enables efficient numerical solvers, reducing the number of inference steps from $100$--$1000$ to $20$--$50$.
		\vfill
		\item SDEs can be reversed in time using the score function $\nabla_{\bx} \log p_t(\bx)$.
		\vfill
		\item The continuous-time score matching objective generalizes DDPM and NCSN to continuous time.
		\vfill
		\item For affine-drift SDEs, the transition kernel $q(\bx(t) | \bx(0))$ is Gaussian, making the training objective tractable.
		\vfill
		\item Discretizing the reverse SDE yields ancestral sampling, while discretizing the PF-ODE yields deterministic sampling.
		\vfill
		\item Flow matching approach suggests fitting the vector field $\bv(\bx, t)$ directly. 
	\end{itemize}
\end{frame}
\end{document}