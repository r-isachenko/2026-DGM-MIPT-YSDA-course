...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{block}{Sampling (Ancestral)}
		\begin{enumerate}
			...
			\]
			\vspace{-0.5cm}
			\item Denoise $\bx_{t - 1} = \bmu_{\btheta, t}(\bx_t) +  \sigma_t \cdot \bepsilon$, $\bepsilon \sim \cN(0, \bI)$.
		\end{enumerate}
	\end{block}
\end{frame}
%=======
...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{block}{Guided Score Function}
		...
		\begin{align*}
			...
			{\color{olive}\bs^{\gamma}_{\btheta, t}(\bx_t, \by)} &= \bs_{\btheta, t}(\bx_t) + {\color{teal}\gamma} \cdot \nabla_{\bx_t} \log p(\by | \bx_t)
		\end{align*}
		\vspace{-0.5cm}
	\end{block}
...
\begin{frame}{Recap of Previous Lecture}
	...
	\end{block}
	\begin{itemize}
		\item Introduce $\by = \varnothing$  for $\bs_{\btheta, t}(\bx_t) = \bs_{\btheta, t}(\bx_t, \varnothing)$.		
		\item Drop the labels $\by$ with some fixed probability.
