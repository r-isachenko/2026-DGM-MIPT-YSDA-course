...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{block}{Theorem}
		\vspace{-0.6cm}
		\[
			\frac{1}{n} \sum_{i=1}^n \KL(q_{\bphi}(\bz| \bx_i) \,\|\, p(\bz)) = \KL({\color{teal}\qagg(\bz) } \,\|\, p(\bz)) + \bbI_{q} [\bx, \bz].
		\]
		\vspace{-0.6cm}
	\end{block}
	\begin{block}{Revisiting the ELBO}
		...
		\begin{multline*}
		    ...
		    - \underbrace{\vphantom{\sum_{i=1}^n} \bbI_q [\bx, \bz]}_{\text{Mutual Information}} - \underbrace{\vphantom{\sum_{i=1}^n} \KL({\color{teal}\qagg(\bz)} \,\|\, {\color{violet}p(\bz)})}_{\text{Marginal KL}}
		\end{multline*}}
		\vspace{-0.4cm}
	\end{block}
	\vspace{-0.7cm}
	\begin{block}{Optimal VAE Prior}
		\vspace{-0.7cm}
		\[
			\KL(\qagg(\bz) \,\|\, p(\bz)) = 0 \ \Leftrightarrow \ p (\bz) = \qagg(\bz) = \frac{1}{n} \sum_{i=1}^n q_{\bphi}(\bz| \bx_i).
		\]
		\vspace{-0.5cm}\\
...
\begin{frame}{Recap of Previous Lecture}
	...
	\begin{itemize}
		\item \textbf{Prior mismatch:} The unimodal encoder $q_{\bphi}(\bz|\bx)$ yields $\qagg(\bz)$ that often does not match the Gaussian prior $p(\bz)$.
...
\begin{frame}{Recap of Previous Lecture}
	...
	\vspace{-0.3cm}
	\begin{itemize}
		\item \textbf{Generator:} a generative model $\bx = \bG_{\btheta}(\bz)$ that produces more realistic samples.
		\item \textbf{Discriminator:} a classifier $D_{\bphi}(\bx) \in [0, 1]$ distinguishing real from generated samples.
	\end{itemize}
\end{frame}
%=======
\begin{frame}{Outline}
