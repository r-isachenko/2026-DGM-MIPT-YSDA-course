...
\begin{frame}{Recap of Previous Lecture}
	\vspace{-0.5cm}
	\begin{multline*}
		\cL_{q, \btheta}(\bx)  =  \bbE_{q} \log \pt(\bx | \bz) - \KL (q(\bz) \| p(\bz)) = \\ = \bbE_q \left[ \log \pt(\bx | \bz) - \log \frac{q(\bz)}{p(\bz)} \right] \rightarrow \max_{q, \btheta}.
	\end{multline*}
	\vspace{-0.5cm}
...
\begin{frame}{Recap of Previous Lecture}
	\vspace{-0.3cm}
	\[
		\cL_{\bphi, \btheta}(\bx)  = \bbE_{q} \log \pt(\bx | \bz) - \KL(q_{\bphi}(\bz| \bx) \| p(\bz)) \rightarrow \max_{\bphi, \btheta}.
	\]
	\vspace{-0.3cm}
...
\begin{frame}{ELBO Surgery}
	...
	\begin{block}{Theorem}
		\vspace{-0.5cm}
		\[
		    \frac{1}{n} \sum_{i=1}^n \KL(q_{\bphi}(\bz| \bx_i) \| p(\bz)) = {\color{violet} \KL(\qagg(\bz) \| p(\bz))} + {\color{teal}\bbI_{q} [\bx, \bz]};
		\]
        \eqpause
		\vspace{-0.5cm}
		\begin{itemize}
			\item $\qagg(\bz) = \frac{1}{n} \sum_{i=1}^n q_{\bphi}(\bz| \bx_i)$ denotes the \textbf{aggregated} variational posterior.
			\item $\bbI_{q} [\bx, \bz]$ is the mutual information between $\bx$ and $\bz$ under the data distribution $\pd(\bx)$ and $q_{\bphi}(\bz| \bx)$.
			\item  {\color{violet} The first term} encourages $\qagg(\bz)$ to match the prior $p(\bz)$.
...
\begin{frame}{ELBO Surgery}
	...
	\begin{block}{Proof}
		...
		\begin{multline*}
		    ...
		    \nextonslide{= \\= \frac{1}{n} \sum_{i=1}^n \int q_{\bphi}(\bz| \bx_i) \log \frac{{\color{violet}\qagg(\bz)} {\color{teal}q_{\bphi}(\bz| \bx_i)}}{{\color{violet}p(\bz)} {\color{teal}\qagg(\bz)}} d \bz}
		    \nextonslide{= \\ = \int \frac{1}{n} \sum_{i=1}^n  q_{\bphi}(\bz| \bx_i) \log {\color{violet}\frac{\qagg(\bz)}{p(\bz)}} d \bz + \frac{1}{n}\sum_{i=1}^n \int q_{\bphi}(\bz| \bx_i) \log {\color{teal}\frac{q_{\bphi}(\bz| \bx_i)}{\qagg(\bz)}} d \bz}
		    \nextonslide{= \\ = \KL (\qagg(\bz) \| p(\bz)) + \frac{1}{n}\sum_{i=1}^n \KL(q_{\bphi}(\bz| \bx_i) \| \qagg(\bz))}
		\end{multline*}
        \eqpause
		}
		\vspace{-0.4cm}
		\[
			\bbI_{q} [\bx, \bz] = \frac{1}{n}\sum_{i=1}^n \KL(q_{\bphi}(\bz| \bx_i) \| \qagg(\bz)).
		\]
	\end{block}
\end{frame}
%=======
\begin{frame}{ELBO Surgery}
	...
	\begin{block}{Revisiting the ELBO}
		...
		\begin{multline*}
		    ...
		    \nextonslide{= \\ = \underbrace{\frac{1}{n} \sum_{i=1}^n \bbE_{q_{\bphi}(\bz| \bx_i)} \log \pt(\bx_i | \bz)}_{\text{Reconstruction Loss}} - \underbrace{\vphantom{ \sum_{i=1}^n} \bbI_q [\bx, \bz]}_{\text{Mutual Information}} - \underbrace{\vphantom{ \sum_{i=1}^n} \KL(\qagg(\bz) \| {\color{teal}p(\bz)})}_{\text{Marginal KL}}}
		\end{multline*}
		}
		\vspace{-0.3cm}
	\end{block}
...
\begin{frame}{Marginal KL}
	...
	\begin{itemize}
		...
		\item It is generally believed that the \textbf{mismatch between} $p(\bz)$  \textbf{and} $\qagg(\bz)$  is the primary explanation for blurry VAE-generated images.
	\end{itemize}
    \eqpause
	\begin{figure}
...
\begin{frame}{Why are VAE Generations Blurry?}
	...
	\begin{block}{Blurriness from Averaging}
		\begin{itemize}
			\item $\pd(\bx)$ and $q_{\bphi}(\bz | \bx)$ give us the aggregated posterior $\qagg(\bz)$.
...
\begin{frame}{Vector Quantized VAE (VQ-VAE): Final algorithm}
	...
	\begin{block}{Training}
		\begin{enumerate}
			\item Sample $\bx \sim \pd(\bx)$.
			\item Compute the encoding $\bz_e = \NN_{e,\bphi}(\bx)$.
			\item Compute the quantized representation (per spatial location if applicable):
			\vspace{-0.3cm}
			\[
			...
			\vspace{-0.3cm}
			\[
				\cL = - \cL_{\bphi, \btheta}(\bx) + \big\|\sg[\bz_e] - \be_{k^*}\big\|_2^2 + \beta \big\|\bz_e - \sg[\be_{k^*}]\big\|_2^2.
			\]
			\vspace{-0.5cm}
...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item The mismatch between $p(\bz)$ and $\qagg(\bz)$ is widely regarded as the principal reason for VAE-generated image blurriness.		
		\vfill
