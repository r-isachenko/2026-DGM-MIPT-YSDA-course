...
try {
 for (const slide of map.filter(s => !selected || selected.includes(s.slide))) {
  await page.goto(`${baseURL}/${slide.slide}`, { waitUntil: 'networkidle' }); await ready()
  ...
  await page.screenshot({ path: `${qa}/slide-${String(slide.slide).padStart(2,'0')}.png` })
 ...
 const annotationSlide = map.find(s => s.frame === 21)
 assert.ok(annotationSlide, 'Missing CoV / LOTUS slide for annotation check')
 const annotationPage = annotationSlide.slide
 await page.goto(`${baseURL}/${annotationPage}?tools`, {waitUntil:'networkidle'}); await ready()
 ...
 const download=await downloadPromise; await download.saveAs(resolve(qa, 'session-test.json'))
 const session=JSON.parse(readFileSync(resolve(qa, 'session-test.json'),'utf8')); assert.ok(session.drawings[String(annotationPage)])
 ...
 await toolbar.locator('input[type=file]').setInputFiles(resolve(qa, 'session-test.json'))
 ...
 await page.screenshot({path:resolve(qa, 'annotation-tools.png')})
 for (let step=0;step<=annotationSlide.clicks;step++) await toolbar.getByRole('button',{name:'Next step',exact:true}).click()
 await page.waitForFunction(n=>location.pathname===`/${n}`, annotationPage + 1)
 await toolbar.getByRole('button',{name:'Previous step',exact:true}).click()
 await page.waitForFunction(n=>location.pathname===`/${n}`, annotationPage)
 const backDownload=page.waitForEvent('download');await toolbar.getByRole('button',{name:'Save',exact:true}).click()
 await (await backDownload).saveAs(resolve(qa, 'session-return-test.json'))
 const returned=JSON.parse(readFileSync(resolve(qa, 'session-return-test.json'),'utf8'))
 assert.equal(returned.drawings[String(annotationPage)],session.drawings[String(annotationPage)],'Drawing changed after clicks / slide return')
 ...
 await page.screenshot({path:resolve(qa, 'last-ui-state.png')})
 writeFileSync(resolve(qa, 'browser-report.json'),JSON.stringify({result,errors,failed},null,2))
 await browser.close()
}
