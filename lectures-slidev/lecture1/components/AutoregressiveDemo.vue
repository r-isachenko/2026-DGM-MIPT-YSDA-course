  ...
  <DemoPanel data-demo="autoregressive">
    <div v-if="!isPrintMode" class="demo-controls">
      <div class="ar-mode" role="group" aria-label="Operation">
        <button :aria-pressed="mode === 'sample'" @click="switchMode('sample')">Sampling</button>
        <button :aria-pressed="mode === 'evaluate'" @click="switchMode('evaluate')">Density evaluation</button>
      </div>
    ...
    <div class="ar-main">
      ...
      <div class="ar-distribution">
        <template v-if="!isPrintMode">
          ...
          <p>{{ mode === 'sample' ? 'Sample the next pixel, then append it to the context.' : 'Read the observed pixel and record its conditional probability.' }}</p>
        </template>
        <template v-else>
          <h2>One model, two operations</h2>
          <p><b>Sampling:</b> sample a pixel from its conditional distribution; append it to the context.</p>
          <p><b>Density evaluation:</b> use the observed prefix; multiply the probabilities of the observed pixels.</p>
        </template>
      </div>
    </div>
    ...
    <div class="demo-takeaway">Sampling builds the image pixel by pixel. Density evaluation uses an image that is already known.</div>
    <div class="demo-note">Toy data: {{ data.length }} binary 3 × 3 images with stripes. Conditional probabilities come from counts.</div>
  </DemoPanel>
</template>
<style scoped>
